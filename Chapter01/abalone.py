...
def abalone_exec(epochs=10, batch_size=10, report=1):
    ...
    train_and_test(epoch, batch_size, report)              # 딥러닝 model 생성 및 전체학습과정 처리
...
def load_abalone_dataset():
    ...
    global data, input_cut, output_cnt    # 전역변수.... -> 고치는게 좋지 않을까?, 계속 사용하기위함이라긴 함
...
def train_and_test(epochs, batch_size, report):
    ...
    for epoch in range(epochs):
        ...
        for n in range(step_size):
            ...
            loss, acc = run_train(train_x, trian_y)
            losses.append(loss)
            accs.append(acc)
...
def get_test_data():
    global data, shuffle_map, test_begin_inx, output_cnt
    test_data = data[shuffle_map[test_begin_inx:]]     # test data 생성
...
def run_trian(x, y):
    ...
    loss, aux_nn = forward_postproc(output, y)       # 순전파, loss 계산(predict과 ground truth 차이 오차)
    ...
    backprop_neuralnet(G_loss, aux_nn)               # 역전파
